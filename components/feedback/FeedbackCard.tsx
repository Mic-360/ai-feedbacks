...
export function FeedbackCard({
  ...
  const num = typeof index === "number" ? String(index).padStart(2, "0") : idShort;

  return (
    <button
      type="button"
      onClick={onClick}
      className="group grid grid-cols-12 items-start gap-4 sm:gap-6 py-6 text-left border-b border-[var(--rule)] transition-colors hover:bg-[rgba(0,0,0,0.025)] w-full"
    >
      {/* Left rail */}
      <div className="col-span-12 sm:col-span-2 flex flex-col gap-1 ms-cap text-[var(--mute)]">
        ...
        <span className={fixCount > 0 ? "text-[var(--accent-sage)]" : "text-[var(--mute)]"}>
